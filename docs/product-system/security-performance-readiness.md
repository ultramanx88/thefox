...
| System program cards | 21 cards checked; every program card has `status`, `expectedState`, `errorState`, `auditEvent`, `rollbackNote`, `verificationCommand`, and `prompt`. |
...
| Data access strategy program | Prisma is the default for type-safe CRUD, state transition, audit write, and migration; Postgres-native SQL powers ledger, reports, read models, heavy joins, and scale controls. | Raw SQL is outside migration, report queries hit transactional tables without budget, ledger/order writes lack transaction/locking rules, or Prisma is used where a bounded read model is required. | No audit expected for architecture docs; future heavy read model rebuild emits `report.*`, `stock.*`, or `settlement.*` audit events. | Revert docs/card only unless runtime changed; runtime data access changes require migration rollback, EXPLAIN evidence, and no direct business data edits. | `rg -n 'Prisma|Postgres-native|materialized view|EXPLAIN|read model|transaction|locking' docs/product-system apps/api/prisma/schema.prisma` | `ทำ program Data access strategy ต่อ: วางกติกา Prisma + Postgres สำหรับ CRUD, transaction, ledger, report, materialized view, raw SQL, index และ EXPLAIN` |
