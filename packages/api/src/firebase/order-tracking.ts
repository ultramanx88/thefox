...
export class OrderTrackingService {
  ...
  private orderSubscriptions: Map<string, Unsubscribe> = new Map();
  private deliverySubscriptions: Map<string, Unsubscribe> = new Map();
  ...
  async updateOrderStatus(update: OrderStatusUpdate): Promise<void> {
    try {
      ...
      if (update.location) {
        updateData.currentLocation = new GeoPoint(
      ...
      if (update.location) {
        statusHistoryEntry.location = new GeoPoint(
          update.location.latitude,
          update.location.longitude
        );
      }
   ...
   */
  private async getOrderStatusHistory(orderId: string): Promise<OrderStatusHistory[]> {
    try {
      const orderRef = doc(db, 'orders', orderId);
