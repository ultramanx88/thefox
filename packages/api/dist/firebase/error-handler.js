...
class FirebaseErrorHandler {
     ...
     */
    async handleFunctionError(error, context) {
        const fullContext = {
            service: 'functions',
            operation: context.operation || 'unknown',
            timestamp: new Date(),
     ...
     */
    async retryOperation(operation, context, customConfig) {
        ...
        // All retries failed
        await this.logError(lastError, context, ErrorSeverity.HIGH);
        throw lastError;
    }
    /**
     * Check if error is retryable
    ...
    categorizeFirestoreError(error) {
